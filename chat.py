...
async def send_data_to_crm(data):
    crm_webhook_url = 'https://b24-kw5z35.bitrix24.by/rest/11/ffqo36u9m5t1zydv/crm.lead.add.json'
    lead_data = {
        'fields': {
...
async def fetch_gpt_response(prompt):
    ...
    2. Уточните, пожалуйста, на какой курс Вы хотели бы записаться? Для записи от Вас необходимы ваши полные ФИО, контактный телефон и электронная почта, на которую мы вышлем письмо-приглашение на курс.
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            messages=[
                {"role": "system",
                 "content": "Act as a friendly manager from https://belhard.academy and respond to user queries using the provided knowledge base. "
